...
### Quality filter (CRITICAL — apply BEFORE presenting any learning):

**Non-inferable test:** Would a competent developer using this stack already know this? If yes → SKIP.

SKIP: generic UI/UX, framework basics, language fundamentals, general best practices, session-specific details with no future value, duplicates of existing skills.

KEEP: project-specific conventions, hidden module dependencies, counter-intuitive business rules, project-specific workarounds, environment quirks unique to this setup.
